...
uint256 hashGenesisBlock("0x9b04501affecb25a658f3dee22a1bca64353618ab0c8374708e27a50bba1e772");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dopecoin: starting difficulty is 1 / 2^12
...
{
    // Dopecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Dopecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Dopecoin: 3.5 days CHANGED = 1day
static const int64 nTargetSpacing = 1 * 60; // Dopecoin: 2.5 minutes CHANGED = 1minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dopecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dopecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb69c0c51da5b234588f218bc8dc28022cc549cc24ab387ca2cbc9c19ceeb5a82");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "24/8/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f5c97dc640aecd75f78ad6a09bd3d6cf790c7f56fe9b036ae1d99c20990b4add4684de90f49ad15517afb1546f8702c4bdb2fa9f5f55821ac5565df047cd2077") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1535119858;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084585211;

        if (fTestNet)
        {
            block.nTime    = 1535119844;
            block.nNonce   = 385398712;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xfbbd30b955b9317f03e421de36d134a17158843ae13b5cc41a0cc56de080e5a0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb2, 0xda }; // Dopecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DopecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DopecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DopecoinMiner(CWallet *pwallet)
{
    printf("DopecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running DopecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DopecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DopecoinMiner, pwallet));
}
